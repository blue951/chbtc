<template>
    <div class="shortcut" style="position:relative;z-index:3;">
        ...
        <Modal v-model="modal6" title="用户登录" :loading="loading" @on-ok="handleSubmit('formValidate')" @on-cancel="handleReset('formValidate')">
        ...
        <Modal v-model="ifShowReg" @on-ok="regSubmit('regForm')" @on-cancel="regReset('regForm')" title="欢迎注册">
            <Form ref="regForm" :model="regForm" :rules="regRules" :label-width="80">
                ...
                <Form-item label="" prop="readFirst">
                    <Checkbox label="吃饭" v-model="regForm.regAgree">
                        我已阅读并同意
                        <router-link to="/">《人人阀用户注册协议》</router-link>
                    </Checkbox>
                </Form-item>
            </Form>
        </Modal>
                    ...
                    jsonpAjax('/portal/reg',{
                        ...
                        response => {
                            ...
                            if(statusCode && statusCode == 0){
                                commit(types.LOGIN_SUCCESS, response.data.status);
                                //this.autoLogin();//注册成功就自动登录
                                this.$Message.info(result.message,5)
                            }else if(statusCode && statusCode == 3){
                                this.$Message.error(result.message,5)
                            }else{
                                this.$Message.info(result.message,5)
                            }
                        },
